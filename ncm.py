...
class NcmClientv3(BaseNcmClient):
    ...
    def new_create_exchange_site(self, *args, **kwargs):
        """
        Creates an exchange site.
        :param kwargs: A set of zero or more allowed parameters
        in the allowed_params list.
        ...
        data = {
            "data": {
                ...
                "attributes": {
                    ...
                    "lan_as_dns": params['filter[lan_as_dns]'],
                    "local_domain": params['filter[local_domain]']
                },
                "relationships": {
                    "exchange_network": {
                        "data": {
                            "id": params['filter[exchange_network_id]'],
                            "type": "exchange_networks"
                        }
                    },
                    "endpoints": {
        ...
        # Check if primary_dns is provided, if not, make an API call to get it
        if 'filter[primary_dns]' in params:
            data["data"]["attributes"]["primary_dns"] = params['filter[primary_dns]']
        else:
           #insert APIv2 call here to get primary DNS
           primary_dns = '192.168.1.1'
           n = NcmClient(api_keys=NcmClient.api_keys)
           config_manager = n.get_configuration_managers(router=params['filter[router_id]'])
           print(config_manager)
           data["data"]["attributes"]["primary_dns"] = primary_dns

        # Check if secondary_dns is provided
        if 'filter[secondary_dns]' in params:
            data["data"]["attributes"]["secondary_dns"] = params['filter[secondary_dns]']
